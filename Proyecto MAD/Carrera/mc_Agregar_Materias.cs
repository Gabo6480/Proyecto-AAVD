...
namespace Proyecto_MAD
{
    public partial class mc_Agregar_Materias : Form
    {
        ...
        public mc_Agregar_Materias(Guid add, bool remove)
        {
            ...
            if (remove)
            {
                ...
                var source = Global.DBManager.careerSubjects(add);
        ...
        private void btn_Agregar_Click(object sender, EventArgs e)
        {
            foreach (DataGridViewRow row in dGV_Materias.Rows)
            {
                if (row.Cells[CheckBox.Name] != null && row.Cells[CheckBox.Name].Value != null)
                {
                    ...
                    {
                        Global.DBManager.careerSubjects(agregar, (int)row.Cells[Clave.Name].Value, row.Cells[Nombre.Name].Value.ToString(), row.Cells[Alias.Name].Value.ToString(), remover);
                    }
                }
            }
            this.Close();
